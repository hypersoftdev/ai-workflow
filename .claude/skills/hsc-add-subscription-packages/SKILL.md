---
name: hsc-add-subscription-packages
description: Hypersoft Claude Skill: Add or extend subscription packages in an app that already has billing (e.g. weekly-only to add monthly and yearly). Audits existing tiers, asks for missing plans and IDs. Use when expanding paywall subscription tiers or /hsc-add-subscription-packages.
---

# Add Subscription Packages

Follow `.claude/rules/21-ads-billing.md`, `00-global.md`, `04-mvi-presentation.md`, `16-logging.md`.
...
For greenfield billing → use `hsc-implement-in-app-billing`. For in-app products → use `hsc-add-inapp-packages`.
...
| App state                                                  | Action                                |
|------------------------------------------------------------|---------------------------------------|
| No `BillingDataSource` / `BillingRepository` / paywall     | Stop → **`hsc-implement-in-app-billing`** |
| Legacy v3 (`BillingPurchaseListener`, `startConnection()`) | Stop → **`hsc-update-in-app-billing`**    |
...
Same rules as `hsc-implement-in-app-billing`:
...
- Re-scaffold billing from scratch (use `hsc-implement-in-app-billing`)
- Add in-app products here (use `hsc-add-inapp-packages`)
