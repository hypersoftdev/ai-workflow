...
**Does not** create UI or Intent-State-Effect — use `hsc-figma-to-xml` or `hsc-figma-to-compose` + `hsc-create-mvi` for those. Presentation / `:feature-*` must **not** depend on `:data`.
...
- If `writeTestsWithFeatures` is `true`: UseCase unit test with `Fake*Repository` (`test/hsc-test-unit`)
...
- [ ] Tell user to wire presentation via `hsc-create-mvi` if UI not done yet
