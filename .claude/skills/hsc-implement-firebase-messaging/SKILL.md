---
name: hsc-implement-firebase-messaging
description: Hypersoft Claude Skill: Add firebase-messaging dependency to core-platform via Version Catalog only. Use when the user asks to add FCM / Firebase Cloud Messaging dependency, firebase-messaging, or /hsc-implement-firebase-messaging — not for MessagingService or push UI.
---
...
- **Mandatory** on every **`hsc-setup-new-project`** bootstrap
- **No** `FirebaseMessagingService`, receivers, manifest entries, tokens, DI, or managers

## Step 1 — Catalog (latest stable)

Add to `libs.versions.toml` under **Firebase** section (`hsc-gradle-organize` headers):
