...
**If `uiFramework` is `compose`:** stop. Use `hsc-figma-to-compose` instead (unless the user explicitly asked to convert this app to XML).

**Never** create Kotlin Fragment/ViewModel/DI here — point user to `hsc-create-mvi` / `hsc-create-clean-architecture` afterward.
...
Dialog / bottom sheet: use `hsc-create-dialog` / `hsc-create-bottom-sheet` (they orchestrate this skill with `dialog_*` / `bottom_sheet_*` names).
...
- Kotlin Fragment/ViewModel (use `hsc-create-mvi` skill)
...
- Tell user to wire via `hsc-create-mvi` (presentation) and `hsc-create-clean-architecture` if new domain/data is needed
