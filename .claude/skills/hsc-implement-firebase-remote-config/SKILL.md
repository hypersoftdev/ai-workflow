---
name: hsc-implement-firebase-remote-config
description: Hypersoft Claude Skill: First-time Firebase Remote Config with SharedPreferences cache, FetchRemoteConfigUseCase, and EntranceFragment / EntranceScreen fetch. Use when adding the RC stack for the first time, fetchAndActivate, or /hsc-implement-firebase-remote-config — not for extra keys on an existing cache (use hsc-add-firebase-remote-config) and not for analytics events.
---

# Implement Firebase Remote Config (first time)

Follow `.claude/rules/22-platform-firebase.md`, `26-data-persistence.md`, `08-gradle.md`, `07-dependency-injection.md`, `16-logging.md`, `00-global.md`.  
SharedPrefs shape: [`.claude/rules/reference/shared-preferences.md`](../../rules/reference/shared-preferences.md).  
Setup overview: `hsc-setup-new-project` Step 8.  
...
| App state                                                            | Action                                                                                                             |
|----------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------|
| DataSource + Repository + prefs cache + Entrance fetch already wired | Stop. Report what exists. New keys → **`hsc-add-firebase-remote-config`**.                                             |
| Stubs from `hsc-setup-new-project`                                       | Complete missing pieces only.                                                                                      |
| No RC                                                                | Continue.                                                                                                          |
...
Latest stable, `# Firebase` / `// Firebase` sections (`hsc-gradle-organize`).
...
- Extra keys on an existing cache (`hsc-add-firebase-remote-config`)
- Analytics / `EventsProvider` (use `hsc-implement-firebase-events`)
