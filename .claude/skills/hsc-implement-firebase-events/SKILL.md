---
name: hsc-implement-firebase-events
description: Hypersoft Claude Skill: First-time Firebase Analytics events for the full app (EventsProvider + screen and/or button posts). Use when adding Analytics events app-wide, EventsProvider, postFirebaseEvent, or /hsc-implement-firebase-events — not for a few screens (use hsc-add-firebase-events) and not for Remote Config.
---
...
For extra screens later → `hsc-add-firebase-events`.

## Entry

| App state                                                              | Action                                                                                |
|------------------------------------------------------------------------|---------------------------------------------------------------------------------------|
| `EventsProvider` + poster + most screens already instrumented          | Stop. Point user to `hsc-add-firebase-events`.                                            |
...
2. Add `fun String.postFirebaseEvent()` to existing `PlatformFirebase` (`:core-platform`) using the [events.md](../events.md) poster (`Param.ITEM_NAME` + `Firebase.analytics`). Create the `object` only if missing — copy `hsc-setup-new-project` [templates/firebase/PlatformFirebase.kt](../hsc-setup-new-project/templates/firebase/PlatformFirebase.kt) (`recordException` + poster + `getDeviceToken`). No `Context` field; no ads-revenue helper in this skill.
...
- Remote Config (`hsc-implement-firebase-remote-config`)
