---
name: hsc-test-unit
description: Hypersoft Claude Skill: Write missing JVM unit tests (UseCases, ViewModels, Flow with fakes), run them, report results; on failures ask consent before fixing production code and retest. Use for unit/MVI/fake-repo tests. Part of test-* suite.
---
...
Follow `.claude/rules/11-testing.md`, `06-coroutines-flow.md`, `18-errors-result.md`.

Obey `.claude/project-settings.json`: if `writeTestsWithFeatures` is `false`, only write tests when the user explicitly asks.
...
- Instrumentation / Espresso for pure business logic → use `hsc-test-e2e` only for user-visible device flows
- Real Room / Retrofit / network / DB → use `hsc-test-integration`
