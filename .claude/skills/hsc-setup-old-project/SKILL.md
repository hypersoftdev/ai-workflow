---
name: hsc-setup-old-project
description: Hypersoft Claude Skill: Migrates an existing production Android app to the hsc-setup-new-project architecture (multi-module Clean Architecture, MVI, Koin lazyModule, XML View Binding or Compose feature modules per uiFramework) while preserving product behavior. Detects MVC, MVVM, Activity-based, single-module, Gradle Groovy, Compose, and older SDK setups, then extracts existing code into the target layers. Use when the user says hsc-setup-old-project, migrate an old/live app, or convert a shipping project to the template. Do not use for greenfield apps — use hsc-setup-new-project.
---
...
**Target architecture:** [hsc-setup-new-project](../hsc-setup-new-project/SKILL.md). Read that skill and treat it as the destination. Reuse its templates (`../hsc-setup-new-project/templates/`). Do not invent a parallel stack.
...
When applied to an existing production Android project, migrate its architecture toward the same structure, patterns, modules, and conventions used by `hsc-setup-new-project`.
...
`hsc-setup-new-project` → Build the architecture from scratch.

`hsc-setup-old-project` → **Preserve the existing product and migrate it to that architecture.**
...
Make the minimum necessary changes to achieve the architecture defined by `hsc-setup-new-project`.

## How this skill differs from `hsc-setup-new-project`

|                | `hsc-setup-new-project`               | `hsc-setup-old-project`                                                                                                     |
|----------------|-----------------------------------|-------------------------------------------------------------------------------------------------------------------------|
...
Execute every `hsc-setup-new-project` step as **migration**: if the slot already exists (prefs helper, API client, Room DB, analytics wrapper), move it into the target module/layer. Create a template file only when that piece is missing.
...
Follow `08-gradle.md` + [reference/gradle.md](../../rules/reference/gradle.md) and **`hsc-gradle-organize`**.

1. Groovy → Kotlin DSL if needed (`settings.gradle.kts`, module scripts, catalog). **Same conversion contract as `hsc-gradle-update` Step 0** — keep both docs aligned when this changes (`MUST_READ_BEFORE_SKILL_CHANGES.md`).
2. If AGP is below **9.3** or `kotlin-android` is applied → bump to **AGP 9.3+** and remove `org.jetbrains.kotlin.android` / `kotlin-kapt` / `android.kotlinOptions` (`hsc-gradle-update` Step 0.5). Built-in Kotlin. Do not set `android.builtInKotlin=false`. Set `compileOptions` to `VERSION_21` and `compileSdk` 37.1. Then migrate R8 (`hsc-gradle-update` Step 0.6).
3. `include` the mandatory module set from `hsc-setup-new-project` (`:app`, `:domain`, `:data`, `:core-common`, `:core-ui`, `:core-platform`; **xml:** also `:presentation`; **compose:** also `:core-design` + `:feature-*` — keep `:gmaAds` / extra modules if they already exist).
4. Move existing dependencies into `libs.versions.toml` **at the same versions**. Do not run a full `hsc-gradle-update` bump. Bump a library only if the new architecture cannot compile without it; tell the user what changed.
...
7. `:app` script shape from `hsc-setup-new-project` Step 1 (`android` section order, `bundle.language.enableSplit = false`, `base.archivesName`).
8. Every module `.gitignore` (`/build`; `:app` also `/release`).
9. Organize sections with **`hsc-gradle-organize`**.
...
Copy Parent*/anim/extensions/Glide/`PlatformFirebase` from [hsc-setup-new-project templates](../hsc-setup-new-project/templates/) **only when missing**. See `hsc-setup-new-project` Steps 2, 6, 7.
...
4. Remote Config: if present, keep **existing keys**; wrap with DataSource + cache-to-prefs (`hsc-setup-new-project` Step 8 / `hsc-implement-firebase-remote-config`). If missing, add the template RC stack.
5. Analytics: keep **event names and params**; move posting into `PlatformFirebase` + `EventsProvider`. Do not rename events.
6. Ads: if existing ads → keep managers / mediation; relocate into `:gmaAds` only if still inside `:app` and structure matches. If no ads → place GitHub `:gmaAds` (package + host remaps). Wire screens only if user said **yes** (`hsc-implement-admob-ads`). **Not** MVI unless the user explicitly asks (`21-ads-billing`).
...
2. Map each feature Activity (or MVC screen) to a Fragment + MVI via **`hsc-create-mvi`** patterns: move existing UI logic into Intent / State / Effect / ViewModel **without changing what the user sees**. Reuse the same XML (View Binding). See [migration.md](migration.md).
...
2. One `:feature-<kebab>` per screen (`hsc-create-mvi` Compose path). Keep existing composables; wrap in `*Screen` / `*ScreenContent`.
...
Follow `hsc-setup-new-project` Step 7 for **placement**:
...
If Messaging (or a `FirebaseMessagingService`) already exists, **keep it** — do not delete push behavior. If FCM was never there, add the **dependency only** (no new service / token UI) per `hsc-implement-firebase-messaging`.
...
`hsc-setup-new-project` Step 9 checklist **plus**:
...
- Use this skill on a greenfield app — use `hsc-setup-new-project`
...
- Full dependency bump (`hsc-gradle-update`) unless required to compile
...
Same as `hsc-setup-new-project`: optional later `/hsc-setup-design-system`; screens via `hsc-figma-to-xml` **or** `hsc-figma-to-compose` → `hsc-create-mvi`; new domain/data via `hsc-create-clean-architecture`; `hsc-review-complete` before PR. Missing RC / Analytics coverage: `hsc-implement-firebase-remote-config`, `hsc-implement-firebase-events`, then `add-firebase-*` for later screens.
