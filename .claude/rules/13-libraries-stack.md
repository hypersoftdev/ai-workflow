...
| UI | **xml:** Material 3, ConstraintLayout, View Binding. **compose:** Compose BOM, Material3, Navigation Compose, Coil 3 |
| Navigation | **xml:** navigation-fragment/ui + Safe Args. **compose:** navigation-compose |
| Lifecycle | viewmodel-ktx, runtime-ktx |
| DI | Koin (default) |
| Coroutines | kotlinx-coroutines-core, play-services integration |
| Firebase (new projects) | BOM + analytics/crashlytics/`firebase-messaging` on `:core-platform`; `firebase-config` + `kotlinx-coroutines-play-services` on `:data` — via `hsc-setup-new-project` (FCM: dependency only, no service) |
| Images | **xml:** Glide via `ImageView.loadImage(...)`. **compose:** Coil 3 `AsyncImage` |
...
- Jetpack Compose — **forbidden when `uiFramework` is `xml`**. Required when `uiFramework` is `compose` (Compose BOM + Navigation Compose + Coil; see `28-compose-ui`)
- Data Binding (View Binding only on xml; neither on compose screens)
...
|------------|-------------|
| Compose UI | XML + View Binding + Material3 when `uiFramework` is `xml`; Compose + Material3 + Coil when `compose` |
