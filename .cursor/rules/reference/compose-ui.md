...
Full detail for `28-compose-ui.mdc`. Canonical app: **AnimeHub** (`E:\SohaibAhmed\Github\AnimeHub`). Do not invent a parallel Compose layout.

Applies when `.cursor/project-settings.json` `uiFramework` is `"compose"`.
...
- `Color.kt` — light/dark scheme colors from design tokens (`hs-setup-design-system`)
- `Type.kt` — `Typography`
