---
name: hs-add-subscription-packages
description: Hypersoft Cursor Skill: Add or extend subscription packages in an app that already has billing (e.g. weekly-only to add monthly and yearly). Audits existing tiers, asks for missing plans and IDs. Use when expanding paywall subscription tiers or /hs-add-subscription-packages.
---
...
For greenfield billing → use `hs-implement-in-app-billing`. For in-app products → use `hs-add-inapp-packages`.
...
| App state                                                  | Action                                |
|------------------------------------------------------------|---------------------------------------|
| No `BillingDataSource` / `BillingRepository` / paywall     | Stop → **`hs-implement-in-app-billing`** |
| Legacy v3 (`BillingPurchaseListener`, `startConnection()`) | Stop → **`hs-update-in-app-billing`**    |
...
Same rules as `hs-implement-in-app-billing`:
...
- Re-scaffold billing from scratch (use `hs-implement-in-app-billing`)
- Add in-app products here (use `hs-add-inapp-packages`)
