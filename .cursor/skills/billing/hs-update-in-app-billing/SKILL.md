---
name: hs-update-in-app-billing
description: Hypersoft Cursor Skill: Migrate existing hypersoft inappbilling from v3.x to v4.0.0 — catalog bump, remove listeners, singleton BillingManager, Flow/suspend API, offerId threading. Use when upgrading inappbilling, Play Billing 9 migration, legacy BillingPurchaseListener, or /hs-update-in-app-billing.
---

# Update In-App Billing (v3 → v4.0.0)

Follow `.cursor/rules/21-ads-billing.mdc`, `00-global.mdc`, `04-mvi-presentation.mdc`, `07-dependency-injection.mdc`, `16-logging.mdc`.
...
Cross-skills: greenfield → `hs-implement-in-app-billing`; add tiers/products after migration → `hs-add-subscription-packages` / `hs-add-inapp-packages`.
...
| App state                                                                                         | Action                                            |
|---------------------------------------------------------------------------------------------------|---------------------------------------------------|
| No billing stack                                                                                  | Stop → **`hs-implement-in-app-billing`**             |
...
| Raw `BillingClient` in features (no hypersoft lib)                                                | Stop — run **`hs-implement-in-app-billing`** instead |
...
After migration, use **`hs-add-subscription-packages`** or **`hs-add-inapp-packages`** to expand SKUs — not this skill.
