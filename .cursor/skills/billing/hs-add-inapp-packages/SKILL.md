---
name: hs-add-inapp-packages
description: Hypersoft Cursor Skill: Add one-time in-app products (non-consumable or consumable) to an app that already has billing. Audits existing products and asks for missing packages and IDs. Use when adding lifetime unlock, credits pack, or /hs-add-inapp-packages.
---
...
For greenfield billing → use `hs-implement-in-app-billing`. For subscription tiers → use `hs-add-subscription-packages`.
...
| Legacy v3 listeners | Stop → **`hs-update-in-app-billing`**    |
...
- Add subscription tiers here (use `hs-add-subscription-packages`)
- Greenfield billing setup (use `hs-implement-in-app-billing`)
