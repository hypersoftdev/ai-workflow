...
Used by `hs-build-debug-apk`, `hs-build-release-apk`, and `hs-build-release-bundle`. Follow `.cursor/rules/14-security-secrets.mdc` and `.cursor/rules/reference/gradle.md`.
...
Required for `hs-build-debug-apk` and `hs-build-release-apk`. Skip for `hs-build-release-bundle`.
...
Required for `hs-build-release-apk` and `hs-build-release-bundle`. Skip for debug.
...
- Installing or launching for `hs-build-release-bundle`
