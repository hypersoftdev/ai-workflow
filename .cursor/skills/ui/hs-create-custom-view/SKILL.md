---
name: hs-create-custom-view
description: Hypersoft Cursor Skill: Create a reusable UI control. XML custom View + View Binding when uiFramework is xml; reusable @Composable when compose. Use when building custom controls, compound views, or widgets beyond standard Material components.
---

# Create Custom View

Follow `.cursor/rules/09-resources-xml.mdc` + [reference/resources-xml.md](../../../rules/reference/resources-xml.md), `19-base-ui.mdc` + [reference/base-ui.md](../../../rules/reference/base-ui.md) when xml; `28-compose-ui.mdc` when compose.

Obey `.cursor/project-settings.json` (`uiFramework`).
...
- **Not** for one-off screen layout — use `ui/hs-figma-to-xml` (xml) or `ui/hs-figma-to-compose` (compose)
