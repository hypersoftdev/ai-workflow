...
**If `uiFramework` is `compose`:** stop. Use `hs-figma-to-compose` instead (unless the user explicitly asked to convert this app to XML).

**Never** create Kotlin Fragment/ViewModel/DI here — point user to `hs-create-mvi` / `hs-create-clean-architecture` afterward.
...
Dialog / bottom sheet: use `hs-create-dialog` / `hs-create-bottom-sheet` (they orchestrate this skill with `dialog_*` / `bottom_sheet_*` names).
...
- Kotlin Fragment/ViewModel (use `hs-create-mvi` skill)
...
- Tell user to wire via `hs-create-mvi` (presentation) and `hs-create-clean-architecture` if new domain/data is needed
