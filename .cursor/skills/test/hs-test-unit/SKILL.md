---
name: hs-test-unit
description: Hypersoft Cursor Skill: Write missing JVM unit tests (UseCases, ViewModels, Flow with fakes), run them, report results; on failures ask consent before fixing production code and retest. Use for unit/MVI/fake-repo tests. Part of test-* suite.
---
...
- Instrumentation / Espresso for pure business logic → use `hs-test-e2e` only for user-visible device flows
- Real Room / Retrofit / network / DB → use `hs-test-integration`
