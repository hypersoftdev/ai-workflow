---
name: hs-setup-new-project
description: Hypersoft Cursor Skill: Bootstrap a new Android multi-module app (domain, data, core-*, xml `:presentation` or compose `:feature-*`) with MainActivity, Entrance start destination, Parent* or Compose NavGraph, PlatformFirebase, Remote Config → SharedPreferences cache, and mandatory firebase-messaging on core-platform. Use when starting a greenfield project. Do not use on an existing production app — use hs-setup-old-project. Confirms and persists project settings first including uiFramework.
---
...
1. **Ads** — always place `:gmaAds` from GitHub (see **Step — Place `:gmaAds`**). Ask: **Implement ads on screens now?**
   - **yes** — after modules/DI exist, run **`hs-implement-admob-ads`** (wire existing screens only; report changed Fragments/`*Screen`s)
...
2. **Firebase Cloud Messaging** is **mandatory** for every new project: add `firebase-messaging` to the catalog + `implementation` on `:core-platform` only (dependency — no `FirebaseMessagingService` or push UI). See **`hs-implement-firebase-messaging`**.
3. **Design system (Figma)** — ask the user to pick:
   ...
   If **a**: save the URL as `figmaDesignSystemUrl` in `project-settings.json`. After Step 6 (`:core-ui` exists), run **`hs-setup-design-system`** with that URL (theme-first tokens in `:core-ui`).  
   If **b**: leave `figmaDesignSystemUrl` empty; scaffold default Material3 `themes.xml` / `colors.xml` in Step 6. User can invoke `/hs-setup-design-system` later.
...
7. If user said **yes** to implement → run **`hs-implement-admob-ads`**. If **no** → stop after place (module only).

## Step 1 — Gradle

Follow `08-gradle.mdc` + [reference/gradle.md](../../../rules/reference/gradle.md) (canonical `:app` / library scripts) and **`hs-gradle-organize`** for catalog + dependency sections.
...
3. Catalog sections/naming per `08-gradle.mdc` / `hs-gradle-organize`
...
9. **Firebase (mandatory):** catalog BOM + analytics / crashlytics / **messaging** on **`:core-platform`**; **config** + `kotlinx-coroutines-play-services` on **`:data`**. Latest stable. **No** `FirebaseMessagingService` / FCM manifest / token UI during setup (see **`hs-implement-firebase-messaging`**)

Organize dependency sections with **`hs-gradle-organize`**.
...
If the user chose design-system **a**, **stop here and run `hs-setup-design-system`** with `figmaDesignSystemUrl` before Step 7. That replaces default Material3 colors/type with Figma tokens and sets `android:windowBackground` — do **not** paint default layouts with `?attr/colorSurface`. If **b**, keep the Step 6 default theme and continue.

## Step 7 — `:core-platform`

### Firebase (catalog + `:core-platform` / `:data`)

Follow **`hs-implement-firebase-messaging`** for FCM. Every new project **must** include Messaging; Analytics + Crashlytics + Remote Config match Speak-Translate.
...
- Ads revenue (`fun Float.logRevenueEvent(context: Context, threshold: Float = 0.1f)`): add when ads screens are wired (`hs-implement-admob-ads` / user said **yes**). Pass `Context` as an argument. Use this app's prefs name / cache key / event string — never copy Speak-Translate `rossPref` / `TaichiTroasCache`
...
**RemoteConfigDataSource** (no dispatcher). Copy [templates/remoteconfig/RemoteConfigDataSource.kt](templates/remoteconfig/RemoteConfigDataSource.kt) (same shape as `hs-implement-firebase-remote-config` templates). Replace `YOUR.PACKAGE`.
...
- [ ] Ads screens wired only if user said **yes** (`hs-implement-admob-ads`); otherwise module-only
...
- [ ] Design system: option **a** ran `hs-setup-design-system` (theme `windowBackground`, no default layout `colorSurface`); or **b** left default Material3 theme

## Do not

- Use on an existing production app — use `hs-setup-old-project`
...
Next: if design system was skipped, optional `/hs-setup-design-system`; then language / onboarding / home via **`hs-figma-to-xml`** (xml) or **`hs-figma-to-compose`** (compose) → `hs-create-mvi`; new domain/data via `hs-create-clean-architecture`; wire Entrance Effects in `nav_graph.xml` (xml) or `NavGraph.kt` (compose). Existing apps missing RC or Analytics: `hs-implement-firebase-remote-config`, `hs-implement-firebase-events`; later screens: `hs-add-firebase-events`; extra RC keys: `hs-add-firebase-remote-config`.
