# hs-setup-old-project — detect and extract

Companion to [SKILL.md](SKILL.md). Target architecture: [hs-setup-new-project](../hs-setup-new-project/SKILL.md).

## Detect → action

| Current setup                                            | Action                                                                                                                   |
|----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------|
| Gradle Groovy (`build.gradle`, `settings.gradle`)        | Convert to Kotlin DSL + Version Catalog. Keep versions. Same contract as **`hs-gradle-update` Step 0** — keep both aligned. |
...
| `isMinifyEnabled` / `proguardFiles` / `proguard-rules.pro` | Migrate to `optimization { enable = true }` on `:app` release + `src/main/keepRules/*.keep` (`hs-gradle-update` Step 0.6). Resource shrinking stays on. |
...
| Firebase Analytics/Crashlytics/RC                        | Relocate to `:core-platform` / `:data` as in hs-setup-new-project. Keep event names and RC keys.                            |
...
4. Fetch Remote Config from Entrance / App startup if that matches `hs-setup-new-project` Step 8 — **non-blocking**; do not delay the old first screen behind a new gate unless the app already waited on RC.
