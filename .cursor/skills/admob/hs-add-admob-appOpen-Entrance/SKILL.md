---
name: hs-add-admob-appOpen-Entrance
description: Hypersoft Cursor Skill: Wire AdMob App Open ENTRANCE placement (consent, blockAppOpen, load with interstitial, showAppOpenOrInterstitialAd). Use for splash/entrance app open, or /hs-add-admob-appOpen-Entrance. If ads were never wired, use hs-implement-admob-ads first. For resume/lifecycle app open use hs-add-admob-appOpen-lifecycle.
---
...
If `:gmaAds` is missing or Entrance was never wired for ads → prefer **`hs-implement-admob-ads`**, or continue here only when module + DI already exist.
...
Sibling: `hs-add-admob-appOpen-lifecycle` for `LIFECYCLE`.
