---
name: hs-add-admob-appOpen-lifecycle
description: Hypersoft Cursor Skill: Wire AdMob App Open LIFECYCLE placement (Dashboard unblock/preload, AppOpenLifecycle or AppOpenLoadingActivity). Use for resume/foreground app open, or /hs-add-admob-appOpen-lifecycle. For splash entrance use hs-add-admob-appOpen-Entrance. If ads were never wired, use hs-implement-admob-ads first.
---
...
If `:gmaAds` is missing → redirect to **`hs-implement-admob-ads`**.
...
Sibling: `hs-add-admob-appOpen-Entrance` for splash `ENTRANCE`.
