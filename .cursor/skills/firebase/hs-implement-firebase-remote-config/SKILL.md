---
name: hs-implement-firebase-remote-config
description: Hypersoft Cursor Skill: First-time Firebase Remote Config with SharedPreferences cache, FetchRemoteConfigUseCase, and EntranceFragment / EntranceScreen fetch. Use when adding the RC stack for the first time, fetchAndActivate, or /hs-implement-firebase-remote-config — not for extra keys on an existing cache (use hs-add-firebase-remote-config) and not for analytics events.
---
...
Setup overview: `hs-setup-new-project` Step 8.  
...
| App state                                                            | Action                                                                                                             |
|----------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------|
| DataSource + Repository + prefs cache + Entrance fetch already wired | Stop. Report what exists. New keys → **`hs-add-firebase-remote-config`**.                                             |
| Stubs from `hs-setup-new-project`                                       | Complete missing pieces only.                                                                                      |
| No RC                                                                | Continue.                                                                                                          |
...
Latest stable, `# Firebase` / `// Firebase` sections (`hs-gradle-organize`).
...
- Extra keys on an existing cache (`hs-add-firebase-remote-config`)
- Analytics / `EventsProvider` (use `hs-implement-firebase-events`)
