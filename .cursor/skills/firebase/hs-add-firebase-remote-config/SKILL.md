---
name: hs-add-firebase-remote-config
description: Hypersoft Cursor Skill: Add one or more Remote Config keys to an existing SharedPreferences cache. Asks the user for value/values (Firebase key, type, default), then matches this app’s SharedPref and Remote Config classes (DataSource or repositories). Use when extending RC cache keys, adding a flag to SharedPrefManager/saveValues, or /hs-add-firebase-remote-config — not for first-time RC wiring (use hs-implement-firebase-remote-config).
---

# Add Firebase Remote Config keys

Follow `.cursor/rules/22-platform-firebase.mdc`, `26-data-persistence.mdc`, `16-logging.mdc`.  
Prefs shape: [`.cursor/rules/reference/shared-preferences.md`](../../rules/reference/shared-preferences.md).  
First-time RC stack → **`hs-implement-firebase-remote-config`**.
...
| App state                                                                 | Action |
|---------------------------------------------------------------------------|--------|
| No RC DataSource / repository **and** no RC cache properties on prefs     | Stop → **`hs-implement-firebase-remote-config`** |
...
- First-time RC stack (`hs-implement-firebase-remote-config`)
- Analytics / `EventsProvider` (`hs-add-firebase-events`)
