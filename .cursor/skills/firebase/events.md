...
Used by `hs-implement-firebase-events` and `hs-add-firebase-events`. Follow `.cursor/rules/22-platform-firebase.mdc`, `12-naming-conventions.mdc`, `16-logging.mdc`.
...
Speak-Translate / `hs-setup-new-project` shape (`PlatformFirebase` in `:core-platform`):
