---
name: hs-implement-firebase-events
description: Hypersoft Cursor Skill: First-time Firebase Analytics events for the full app (EventsProvider + screen and/or button posts). Use when adding Analytics events app-wide, EventsProvider, postFirebaseEvent, or /hs-implement-firebase-events — not for a few screens (use hs-add-firebase-events) and not for Remote Config.
---
...
For extra screens later → `hs-add-firebase-events`.

## Entry

| App state                                                              | Action                                                                                |
|------------------------------------------------------------------------|---------------------------------------------------------------------------------------|
| `EventsProvider` + poster + most screens already instrumented          | Stop. Point user to `hs-add-firebase-events`.                                            |
...
2. Add `fun String.postFirebaseEvent()` to existing `PlatformFirebase` (`:core-platform`) using the [events.md](../events.md) poster (`Param.ITEM_NAME` + `Firebase.analytics`). Create the `object` only if missing — copy `hs-setup-new-project` [templates/firebase/PlatformFirebase.kt](../../project/hs-setup-new-project/templates/firebase/PlatformFirebase.kt) (`recordException` + poster + `getDeviceToken`). No `Context` field; no ads-revenue helper in this skill.
...
- Remote Config (`hs-implement-firebase-remote-config`)
