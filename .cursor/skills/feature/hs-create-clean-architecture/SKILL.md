---
name: hs-create-clean-architecture
description: Hypersoft Cursor Skill: Add domain, data, and core pieces for a feature (UseCases, repository interfaces/impls, DataSources, DI). No UI (no Fragment/XML/Compose Screen). Use when a screen needs new business/data capability, or after hs-create-mvi when domain is missing. Follows Retrofit/Room/SharedPreferences rules references.
---
...
**Does not** create UI or Intent-State-Effect — use `hs-figma-to-xml` or `hs-figma-to-compose` + `hs-create-mvi` for those. Presentation / `:feature-*` must **not** depend on `:data`.
...
- If `writeTestsWithFeatures` is `true`: UseCase unit test with `Fake*Repository` (`test/hs-test-unit`)
...
- [ ] Tell user to wire presentation via `hs-create-mvi` if UI not done yet
