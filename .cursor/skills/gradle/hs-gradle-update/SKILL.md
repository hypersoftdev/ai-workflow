---
name: hs-gradle-update
description: Hypersoft Cursor Skill: Bump all project dependencies to latest stable — AGP 9.3+ with built-in Kotlin (remove kotlin-android / kotlin-kapt). Migrate R8 to optimization { enable = true } + src/main/keepRules. Convert Groovy → Kotlin DSL when needed, bump catalog versions and any hardcoded group:artifact:version in modules. Migrate hardcodes into libs.versions.toml (create catalog if missing) and place deps under hs-gradle-organize sections. Does not add brand-new libraries without approval.
---
...
This skill **bumps versions**. Section order / catalog layout → **`hs-gradle-organize`** (run its logic as part of this flow when hardcodes or section placement are wrong). Groovy `*.gradle` → Kotlin DSL → do that **before** bumping (same conversion contract as `hs-setup-old-project` Step 2). Then **AGP 9.3+ / built-in Kotlin** (Step 0.5) and **R8 `optimization` / `keepRules`** (Step 0.6) before or with the AGP bump.

When you change this skill’s Gradle behavior, also update the Gradle cluster: `hs-gradle-organize`, `hs-setup-old-project` (+ `migration.md`), `08-gradle` + `reference/gradle.md`, and `.claude` twins (`MUST_READ_BEFORE_SKILL_CHANGES.md`).
...
2. Prefer Version Catalog (`libs.versions.toml`) shape from **`hs-gradle-organize`** / [reference/gradle.md](../../../rules/reference/gradle.md)
...
5. Same conversion rules as `hs-setup-old-project` Step 2 / [migration.md](../../project/hs-setup-old-project/migration.md) — keep those docs aligned when this step changes
...
- Every key under `[versions]` in `libs.versions.toml` (create the file with `[versions]` / `[plugins]` / `[libraries]` if absent — see `hs-gradle-organize` / [reference/gradle.md](../../../rules/reference/gradle.md))
...
If the catalog file does not exist → **create** `gradle/libs.versions.toml` in `hs-gradle-organize` format, wire Version Catalog if needed, then migrate.
...
### 5 — Apply `hs-gradle-organize` placement

After bumps/migrations:

- Project modules first, then library sections with exact headers from `hs-gradle-organize`
